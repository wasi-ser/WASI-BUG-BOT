...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +923347748038 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')

         // Ask again when entering the wrong number
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +923347748038")))
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} Telegram CHANNEL:@Musa_x0`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: Private `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: private `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 923347748038`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: @Musa_x0\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
